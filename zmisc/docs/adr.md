...
# NCBI and SRA Data Preparation
## Context and Technology Choice
The ultimate goal of this project is to publish the data generated by this pipeline. While a microbiology resource announcement is one of the intended goals, due to the number and size of the specific datasets involved, it was of interest to develop a portion of the pipeline dedicated to preparation of generated datasets for submission to  archival resources such as NCBI, GenBank, SRA, etc. Outputs would potentially include BioSample metadata tables, SRA metadata tables, mitogenome submission metadata, validation reports, and organized submission directories containing files required for archival review. 

Automated submission to NCBI would be the best case scenario and is considered as a possible extension - depending on development of the project. Pipelines often require user authentication, metadata validation, and manual review, but NCBI provides a CLI-based interface and automated tooling for portions of the archival process. As a result, CLI-assisted submission will be investigated as a potential enhancement if the timeline of the project permits after completion of core functionality.

Archival preparation would be integrated after assembly and annotation processing and before final reporting. Submission-related outputs would be generated within a dedicated submission directory under each pipeline output. 

## Justifications
Preparation of archival materials provides significant practical value while remaining fairly achievable by delivery date of this project. Resulting deliverables would reduce the manual effort associated with publication and data deposition and avoids the risk of making successful project completion dependent upon external submission systems.
