"""Phylogeny dataset generation rule using the final rescued assembly."""
...
rule generate_alignment_dataset:
    input:
        assembly_fasta=str(JOB_DIR / "assembly" / "{sample}" / "resolved.fasta"),
        assembly_done=str(JOB_DIR / "assembly" / "{sample}" / "rescue.done"),
        top_hits=str(JOB_DIR / "phylogeny" / "{sample}" / "top_hits.tsv"),
        top_hits_done=str(JOB_DIR / "phylogeny" / "{sample}" / "top_hits.done")

    output:
        dataset=str(JOB_DIR / "phylogeny" / "{sample}" / "dataset.fasta"),
        done=str(JOB_DIR / "phylogeny" / "{sample}" / "dataset.done")

    params:
        entrez_email=config["tools"]["blast"]["entrez_email"],
        entrez_api_key=config["tools"]["blast"].get("entrez_api_key") or "",
        log_file=str((Path.cwd() / JOB_DIR / "phylogeny" / "{sample}" / "dataset.log").resolve())
