...
rule annotation:
    input:
        ...
        mitos2_done=str(
            JOB_DIR / "setup" / "mitos2" / "mitos2.done"
        )
    ...
    params:
        ...
        genetic_code=config["tools"]["mitos2"].get(
            "genetic_code",
            2,
        ),
        refdir=str(
            Path(config["tools"]["mitos2"]["refdir"]).resolve()
        ),
        refseqver=config["tools"]["mitos2"]["refseqver"],
        circular=str(
            config["tools"]["mitos2"].get("circular", True)
        ),
        noplots=str(
            config["tools"]["mitos2"].get("noplots", False)
        ),
        best=str(
            config["tools"]["mitos2"].get("best", False)
        ),
        ncbicode=str(
            config["tools"]["mitos2"].get("ncbicode", False)
        )
