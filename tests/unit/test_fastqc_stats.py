...
def test_parse_fastqc_summary_valid_zip(tmp_path):
    ...
    assert isinstance(stats, FastQCStats)
...
def test_fastqc_stats_to_dict(tmp_path):
    """Unit test confirming FastQCStats serializes module statuses."""
    ...
    assert data["qc_stage"] == "raw"
    assert data["source_file"] == "sample_001_R1.fastq.gz"
    assert data["overall_status"] == "WARN"
    assert data["module_statuses"] == "Basic Statistics=PASS; Adapter Content=WARN"
